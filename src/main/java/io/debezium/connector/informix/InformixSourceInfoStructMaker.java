...
public class InformixSourceInfoStructMaker extends AbstractSourceInfoStructMaker<SourceInfo> {
    ...
    public void init(String connector, String version, CommonConnectorConfig connectorConfig) {
        ...
        schema = commonSchemaBuilder().name("io.debezium.connector.informix.Source").field(SourceInfo.SCHEMA_NAME_KEY, Schema.STRING_SCHEMA)
                .field(SourceInfo.TABLE_NAME_KEY, Schema.STRING_SCHEMA).field(SourceInfo.COMMIT_LSN_KEY, Schema.OPTIONAL_STRING_SCHEMA)
                .field(SourceInfo.CHANGE_LSN_KEY, Schema.OPTIONAL_STRING_SCHEMA).field(SourceInfo.TX_ID, Schema.OPTIONAL_STRING_SCHEMA)
