...
public class InformixSnapshotChangeEventSource extends RelationalSnapshotChangeEventSource<InformixPartition, InformixOffsetContext> {
    ...
    public InformixSnapshotChangeEventSource(InformixConnectorConfig connectorConfig, MainConnectionProvidingConnectionFactory<InformixConnection> connectionFactory,
                                             InformixDatabaseSchema schema, EventDispatcher<InformixPartition, TableId> dispatcher, Clock clock,
    ...
    protected void determineSnapshotOffset(RelationalSnapshotContext<InformixPartition, InformixOffsetContext> ctx, InformixOffsetContext previousOffset) {
        ...
        if (offset == null) {
            if (previousOffset != null) {
                ...
                offset = new InformixOffsetContext(connectorConfig, TxLogPosition.clone(previousOffset.getChangePosition()),
                        previousOffset.isSnapshotRunning() || previousOffset.isSnapshotCompleted(), previousOffset.isSnapshotCompleted());
            }
            else {
                offset = new InformixOffsetContext(connectorConfig, TxLogPosition.valueOf(Lsn.valueOf(0x00L)), false, false);
            }
            ctx.offset = offset;
        }
    }
    ...
    protected void readTableStructure(ChangeEventSourceContext sourceContext, RelationalSnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext,
        ...
        for (String schema : schemas) {
            ...
            jdbcConnection.readSchema(snapshotContext.tables, null, schema, connectorConfig.getTableFilters().dataCollectionFilter(), null, false);
        }
    }
