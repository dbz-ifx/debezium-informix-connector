...
public class InformixSnapshotChangeEventSource extends RelationalSnapshotChangeEventSource<InformixPartition, InformixOffsetContext> {
    ...
    public InformixSnapshotChangeEventSource(InformixConnectorConfig connectorConfig, InformixConnection jdbcConnection,
                                             ...
                                             EventDispatcher<InformixPartition, TableId> dispatcher, Clock clock, SnapshotProgressListener snapshotProgressListener) {
    ...
    @Override
    protected SnapshottingTask getSnapshottingTask(InformixPartition partition, InformixOffsetContext previousOffset) {
    ...
    @Override
    protected void lockTablesForSchemaSnapshot(ChangeEventSourceContext sourceContext, RelationalSnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext)
            throws SQLException, InterruptedException {
        if (connectorConfig.getSnapshotIsolationMode() == InformixConnectorConfig.SnapshotIsolationMode.READ_UNCOMMITTED) {
            jdbcConnection.connection().setTransactionIsolation(Connection.TRANSACTION_READ_UNCOMMITTED);
            LOGGER.info("Schema locking was disabled in connector configuration");
        }
        else if (connectorConfig.getSnapshotIsolationMode() == InformixConnectorConfig.SnapshotIsolationMode.READ_COMMITTED) {
        ...
        connectorConfig.getSnapshotIsolationMode() == InformixConnectorConfig.SnapshotIsolationMode.REPEATABLE_READ) {
    ...
    protected void releaseSchemaSnapshotLocks(RelationalSnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext) throws SQLException {
        ...
        if (connectorConfig.getSnapshotIsolationMode() == InformixConnectorConfig.SnapshotIsolationMode.REPEATABLE_READ) {
    ...
    @Override
    protected void determineSnapshotOffset(RelationalSnapshotContext<InformixPartition, InformixOffsetContext> ctx, InformixOffsetContext previousOffset) throws Exception {
    ...
    @Override
    protected void readTableStructure(ChangeEventSourceContext sourceContext, RelationalSnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext,
    ...
    @Override
    protected SchemaChangeEvent getCreateTableEvent(RelationalSnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext, Table table) throws SQLException {
    ...
    @Override
    protected void complete(SnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext) {
        try {
     ...
     */
    @Override
    protected Optional<String> getSnapshotSelect(RelationalSnapshotContext<InformixPartition, InformixOffsetContext> snapshotContext, TableId tableId, List<String> columns) {
        String from = columns != null && !columns.isEmpty() ? String.join(", ", columns) : "*";
        return Optional.of(String.format("SELECT %s FROM %s.%s", from, tableId.schema(), tableId.table()));
    }
    ...
    private static class InformixSnapshotContext extends RelationalSnapshotContext<InformixPartition, InformixOffsetContext> {
        ...
        public InformixSnapshotContext(InformixPartition partition, String catalogName) throws SQLException {
