...
import static io.debezium.storage.kafka.history.KafkaSchemaHistory.BOOTSTRAP_SERVERS;
import static io.debezium.storage.kafka.history.KafkaSchemaHistory.TOPIC;
...
public class InformixConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public static final Field SNAPSHOT_MODE = Field.create("snapshot.mode").withDisplayName("Snapshot mode").withEnum(SnapshotMode.class, SnapshotMode.INITIAL)
            .withGroup(Field.createGroupEntry(Field.Group.CONNECTOR_SNAPSHOT, 0)).withWidth(Width.SHORT).withImportance(Importance.LOW)
    ...
    public static final Field SNAPSHOT_ISOLATION_MODE = Field.create("snapshot.isolation.mode").withDisplayName("Snapshot isolation mode")
            .withEnum(SnapshotIsolationMode.class, SnapshotIsolationMode.REPEATABLE_READ).withGroup(Field.createGroupEntry(Field.Group.CONNECTOR_SNAPSHOT, 1))
            .withWidth(Width.SHORT).withImportance(Importance.LOW)
            .withDescription("Controls which transaction isolation level is used and how long the connector locks the monitored tables. " + "The default is '"
                    ...
                    + "' mode no table locks or any *long-lasting* row-level locks are acquired, but connector does not guarantee snapshot consistency." + "In '"
    ...
    public static final Field CDC_BUFFERSIZE = Field.create("cdc.buffersize").withDisplayName("CDC Engine buffer size").withType(ConfigDef.Type.INT)
            .withGroup(Field.createGroupEntry(Field.Group.CONNECTOR_ADVANCED, 0)).withWidth(Width.MEDIUM).withImportance(Importance.MEDIUM)
            .withDescription("Size of the read buffer for receiving events from the server, in bytes.").withValidation(Field::isNonNegativeInteger)
            .withDefault(DEFAULT_CDC_BUFFERSIZE);
    protected static final int DEFAULT_CDC_TIMEOUT = 5;
    public static final Field CDC_TIMEOUT = Field.create("cdc.timeout").withDisplayName("CDC Engine timeout").withType(ConfigDef.Type.INT)
            .withGroup(Field.createGroupEntry(Field.Group.CONNECTOR_ADVANCED, 0)).withWidth(Width.MEDIUM).withImportance(Importance.MEDIUM)
            ...
            .withValidation(Field::isNonNegativeInteger).withDefault(DEFAULT_CDC_TIMEOUT);
    private static final ConfigDefinition CONFIG_DEFINITION = HistorizedRelationalDatabaseConnectorConfig.CONFIG_DEFINITION.edit().name("Informix")
            .type(HOSTNAME, PORT, USER, PASSWORD, DATABASE_NAME)
            .connector(SNAPSHOT_MODE, SNAPSHOT_ISOLATION_MODE, INCREMENTAL_SNAPSHOT_CHUNK_SIZE, CDC_BUFFERSIZE, CDC_TIMEOUT, BINARY_HANDLING_MODE)
            .events(SOURCE_INFO_STRUCT_MAKER).history(TOPIC, BOOTSTRAP_SERVERS).excluding(COLUMN_INCLUDE_LIST, COLUMN_EXCLUDE_LIST).create();
    /**
    ...
    public InformixConnectorConfig(Configuration config) {
        super(InformixConnector.class, config, new SystemTablesPredicate(), t -> Strings.join(".", t.parts()), false, ColumnFilterMode.SCHEMA, false);
